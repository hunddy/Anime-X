module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        let programmerauthor = db.fetch(`education_${message.guild.id}_${message.author.id}`)
        ...
        db.add(`animebucks_${message.guild.id}_${message.author.id}`, programmeramount)
        } else if(args[0] == 'constructor') {
         let constructoramount = Math.floor(Math.random() * 200) + 1; // 1-500 random number. whatever you'd like
        let constructorauthor = db.fetch(`education_${message.guild.id}_${message.author.id}`)
        ...
        db.add(`animebucks_${message.guild.id}_${message.author.id}`, constructoramount)
        } else if(args[0] == 'cooker') {
        let cookeramount = Math.floor(Math.random() * 50) + 1;
        let cookerauthor = db.fetch(`education_${message.guild.id}_${message.author.id}`)
        ...
        db.add(`animebucks_${message.guild.id}_${message.author.id}`, policemenamount)
        }
